...
import { DeleteBranchModal } from "@/app/(dashboard)/admin/branches/DeleteBranchModal";
...
export default function ExecutiveBranchesClientPage({ branches, stats }: BranchesProps) {
                            ...
                            {filteredBranches.map((branch) => (
                                <tr key={branch.id} className="group hover:bg-[var(--color-primary)]/[0.02] transition-colors">
                                    ...
                                    <td className="px-6 py-3 text-right">
                                        <ActionDropdown 
                                            actions={[
                                                { label: "Edit Branch", icon: "edit_square", onClick: () => setEditingBranch(branch) },
