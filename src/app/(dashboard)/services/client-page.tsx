        ...
        <div className="flex-1 p-8 lg:p-12 overflow-y-auto bg-[var(--color-bg-light)] dark:bg-[var(--color-bg-dark)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="glass-card p-5">
                    <p className="text-[var(--text-muted)] opacity-60 text-[10px] font-bold uppercase tracking-widest mb-1">Total Services</p>
                ...
                <div className="glass-card p-5">
                    <p className="text-[var(--text-muted)] opacity-60 text-[10px] font-bold uppercase tracking-widest mb-1">Active Treatments</p>
                    <p className="text-3xl font-sans font-black text-[var(--text-main)]">{stats.active}</p>
                    <div className="flex items-center gap-1 text-[var(--text-muted)] opacity-40 text-[10px] font-bold mt-2 uppercase tracking-widest">
                        <span className="material-symbols-outlined text-sm">check_circle</span>
                        <span>{stats.total > 0 ? Math.round((stats.active / stats.total) * 100) : 0}% Availability</span>
                    </div>
                </div>
                <div className="glass-card p-5">
                    <p className="text-[var(--text-muted)] opacity-60 text-[10px] font-bold uppercase tracking-widest mb-1">Avg. Duration</p>
                ...
                <div className="glass-card p-5">
                    <p className="text-[var(--text-muted)] opacity-60 text-[10px] font-bold uppercase tracking-widest mb-1">Most Popular</p>
            ...
            <div className="bg-[var(--bg-card)] rounded-3xl border border-[var(--border-main)] overflow-hidden shadow-sm">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-[var(--bg-surface-muted)]">
                                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-widest">Service ID</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-widest">Service Name</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-widest">Category</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-widest text-right">Price</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-widest">Duration</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-widest">Status</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-[var(--text-muted)] opacity-60 uppercase tracking-widest text-center">Actions</th>
                            </tr>
                        </thead>
                            ...
                            {filtered.map((service) => (
                                <tr key={service.id} className="hover:bg-[var(--bg-surface-muted)]/50 transition-colors group">
                                    <td className="px-6 py-5">
                                        <span className="font-mono text-[10px] font-bold text-[var(--text-muted)] opacity-40 uppercase truncate max-w-[80px] block">
                                            #{service.id.slice(-6)}
                                        </span>
                                    </td>
                                    <td className="px-6 py-5">
                                    ...
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-2 text-[var(--text-main)] font-display font-medium">
                                            <span className="material-symbols-outlined text-lg opacity-40">timer</span>
                                            <span className="text-sm">{service.duration} min</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-2">
                                            <span className={`size-2 rounded-full ${service.status === 'ACTIVE' ? 'bg-[var(--color-primary)] animate-pulse shadow-[0_0_8px_var(--color-primary)]' : 'bg-[var(--text-muted)] opacity-30'}`}></span>
                                            <span className={`text-[10px] font-black uppercase tracking-widest ${service.status === 'ACTIVE' ? 'text-[var(--color-primary)]' : 'text-[var(--text-muted)] opacity-40'}`}>
                ...
                <div className="px-6 py-5 border-t border-[var(--border-muted)] flex items-center justify-between bg-[var(--bg-surface-muted)]">
                    <p className="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest opacity-40 italic">Showing {filtered.length} of {services.length} services</p>
                </div>
            </div>

            {/* Add/Edit Modal */}
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => { setShowAddModal(false); setEditingService(null); }}>
