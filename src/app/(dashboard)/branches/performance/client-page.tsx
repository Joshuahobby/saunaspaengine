            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex items-baseline gap-2">
                        <h3 className="text-3xl font-bold text-slate-900 dark:text-slate-100">{stats.totalRevenue.toLocaleString()} RWF</h3>
                    ...
                    <p className="text-slate-400 text-xs mt-4">Vs. {(stats.totalRevenue * 0.9).toLocaleString()} RWF last quarter</p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex items-baseline gap-2">
                        <h3 className="text-3xl font-bold text-slate-900 dark:text-slate-100">{stats.occupancyRate}%</h3>
                        <span className="text-[var(--color-primary)] text-sm font-bold flex items-center">
                            <span className="material-symbols-outlined text-sm">trending_up</span> 3.1%
                        </span>
                    </div>
                    <p className="text-slate-400 text-xs mt-4">Across {stats.activeBranches} worldwide locations</p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex items-baseline gap-2">
                        <h3 className="text-3xl font-bold text-slate-900 dark:text-slate-100">+{stats.membershipGrowth}%</h3>
                ...
                <div className="xl:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col">
                    <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            <span className="px-2 py-1 bg-[rgba(19,236,164,0.1)] text-[var(--color-primary)] text-[10px] font-bold rounded uppercase">{stats.activeBranches} Active</span>
                            <span className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 text-[10px] font-bold rounded uppercase">{stats.pendingBranches} Pending</span>
                        </div>
                    </div>
                    <div className="flex-1 min-h-[400px] relative bg-slate-100 dark:bg-slate-950/50 rounded-b-xl overflow-hidden">
                        ...
                        <div className="absolute inset-0 opacity-20 dark:opacity-10 grayscale pointer-events-none bg-[url('https://placeholder.pics/svg/300')] bg-cover bg-center"></div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col">
                    ...
                    <div className="p-4 flex flex-col gap-4 overflow-y-auto max-h-[400px]">
                        {leaderboard.length === 0 ? (
                            <div className="text-center text-slate-500 py-8 text-sm">No branches available</div>
                        ) : (
                            leaderboard.map((branch, idx) => (
                                <div key={branch.id} className={`flex items-center gap-4 p-3 rounded-lg border transition-colors ${idx === 0 ? "border-[var(--color-primary)]/20 bg-[rgba(19,236,164,0.05)]" : "border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50"}`}>
                                    <div className={`h-10 w-10 shrink-0 font-bold rounded-lg flex items-center justify-center ${idx === 0 ? "bg-[var(--color-primary)] text-[#102220]" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"}`}>
                                        {idx + 1}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-slate-900 dark:text-slate-100 truncate">{branch.name}</p>
                                        <p className="text-xs text-slate-500">Revenue: {branch.revenue > 1000000 ? `${(branch.revenue / 1000000).toFixed(1)}M` : branch.revenue.toLocaleString()} RWF</p>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <div className={`flex items-center gap-1 justify-end ${idx === 0 ? "text-[var(--color-primary)]" : "text-slate-600 dark:text-slate-400"}`}>
                                            <span className="material-symbols-outlined text-sm">star</span>
                                            <span className="text-xs font-bold">{branch.csatScore.toFixed(1)}</span>
                                        </div>
                    ...
                    <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 1000 250">
                        <defs>
                            <linearGradient id="primaryGradient" x1="0%" x2="0%" y1="0%" y2="100%">
                                <stop offset="0%" className="performance-chart-gradient-top"></stop>
                                <stop offset="100%" className="performance-chart-gradient-bottom"></stop>
                            </linearGradient>
                        </defs>
                        {/* Revenue Line Area */}
