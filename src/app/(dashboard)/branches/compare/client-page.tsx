    ...
    return (
        <main className="flex flex-col flex-1 gap-12 max-w-[1440px] mx-auto w-full px-6 lg:px-10 py-12 overflow-y-auto">
            ...
            <div className="flex flex-col gap-3 border-b border-[var(--border-muted)] pb-8">
                <h1 className="text-4xl font-display font-bold text-[var(--text-main)]">Branch Comparison</h1>
                <p className="text-lg text-[var(--text-muted)] italic font-medium opacity-80">
                    Comparing performance across all {branches.length} locations.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {branches.map((biz) => (
                    <div key={biz.id} className="bg-[var(--bg-card)] rounded-[2.5rem] border border-[var(--border-muted)] p-10 shadow-sm transition-all hover:shadow-lg hover:border-[var(--color-primary)]/30 group">
                        <div className="flex items-center justify-between mb-8 pb-8 border-b border-[var(--border-muted)]">
                            <div>
                                <h2 className="text-3xl font-display font-bold text-[var(--text-main)] group-hover:text-[var(--color-primary)] transition-colors">{biz.name}</h2>
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-3xl font-display font-black text-[var(--text-main)] tracking-tighter">{formatCurrency(biz.revenue)}</p>
                            </div>
                        </div>
                ...
                {branches.length === 0 && (
                    <div className="col-span-full py-20 text-center">
                        ...
                        <h3 className="text-2xl font-display font-bold text-[var(--text-main)]">No Entities Found</h3>
