            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        onClick={onClose}
                        className="absolute inset-0 bg-[#050706]/80 backdrop-blur-md"
                    />
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-4xl bg-[#0f1412] border border-[var(--border-muted)] rounded-[2.5rem] shadow-2xl overflow-hidden"
                    >
                        <div className="p-8 lg:p-10 space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {availablePackages.map((plan) => (
                                    ...
                                    >
                                        ...
                                        <div className="space-y-1">
                                            <h3 className="font-serif font-black italic text-lg text-white group-hover/plan:text-[var(--color-primary)] transition-colors">{plan.name}</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-[var(--border-muted)]/30">
                                <div className="space-y-3">
                                    ...
                                    <select 
                                        ...
                                        className="w-full h-14 bg-[var(--bg-surface-muted)]/10 border border-[var(--border-muted)] rounded-2xl px-6 text-sm font-bold text-white outline-none focus:border-[var(--color-primary)]/50 transition-all appearance-none"
                                    >
                                        <option value="Monthly" className="bg-[#0f1412]">Monthly Billing</option>
                                        <option value="Yearly" className="bg-[#0f1412]">Yearly Billing (Discounted)</option>
                                    </select>
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <select 
                                        ...
                                        className="w-full h-14 bg-[var(--bg-surface-muted)]/10 border border-[var(--border-muted)] rounded-2xl px-6 text-sm font-bold text-white outline-none focus:border-[var(--color-primary)]/50 transition-all appearance-none"
                                    >
                                        <option value="ACTIVE" className="bg-[#0f1412]">Active / Delighted</option>
                                        <option value="EXPIRING" className="bg-[#0f1412]">Expiring / At Risk</option>
                                        <option value="CANCELLED" className="bg-[#0f1412]">Cancelled / Inactive</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 pt-4">
                                <button
                                    onClick={onClose}
                                    className="h-14 flex-1 rounded-2xl border border-[var(--border-muted)] text-[var(--text-muted)] font-bold tracking-widest uppercase text-xs hover:bg-[var(--bg-surface-muted)]/5 transition-all"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className="h-14 flex-[2] rounded-2xl bg-white text-black font-bold tracking-[0.2em] uppercase text-xs hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 group disabled:opacity-50"
                                >
                                    {isLoading ? (
