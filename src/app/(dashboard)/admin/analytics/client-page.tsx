        ...
        <div className="flex flex-col gap-8 px-4 lg:px-6 py-6 max-w-[1600px] mx-auto w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {kpiCards.map((card, idx) => (
                    <motion.div
                        ...
                        key={card.label}
                        className="group relative rounded-[var(--r-lg)] border border-[var(--border-muted)] bg-[var(--bg-card)]/40 p-8 shadow-sm backdrop-blur-sm transition-all duration-700 hover:border-[var(--color-primary)]/40 overflow-hidden"
                    >
                        ...
                        <div className="space-y-0.5">
                            ...
                            <h3 className="text-3xl font-display font-black text-white tracking-tighter">{card.value}</h3>
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <motion.div
                    ...
                    className="lg:col-span-8 rounded-[var(--r-lg)] border border-[var(--border-muted)] bg-[var(--bg-card)]/40 p-1 backdrop-blur-sm overflow-hidden group shadow-sm min-w-0"
                >
                    <div className="p-8 pb-0">
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[var(--border-muted)] pb-6 relative z-10">
                            <div className="space-y-1">
                                <h3 className="text-xl font-display font-bold text-[var(--text-main)]">Platform Growth</h3>
                ...
                <motion.div
                    ...
                    className="lg:col-span-4 rounded-[var(--r-lg)] border border-[var(--border-muted)] bg-[var(--bg-card)]/40 p-8 backdrop-blur-sm shadow-sm flex flex-col gap-6 relative overflow-hidden min-w-0"
                >
                    ...
                    <div className="space-y-1 border-b border-[var(--border-muted)] pb-6 relative z-10">
                        <h3 className="text-xl font-display font-bold text-[var(--text-main)]">Subscription Breakdown</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <motion.div
                    ...
                    className="lg:col-span-7 rounded-[var(--r-lg)] border border-[var(--border-muted)] bg-[var(--bg-card)]/40 overflow-hidden shadow-sm flex flex-col backdrop-blur-sm min-w-0"
                >
                    <div className="p-8 border-b border-[var(--border-muted)] flex justify-between items-center bg-black/10">
                        <div className="space-y-1">
                            <h3 className="text-xl font-display font-bold text-[var(--text-main)]">Top Businesses by Revenue</h3>
                        ...
                        {topCorporates.length > 0 ? topCorporates.map((corp, idx) => (
                            <div key={idx} className="flex items-center justify-between px-8 py-4 hover:bg-[var(--color-primary)]/[0.03] transition-all group/row cursor-pointer border-b border-[var(--border-muted)]/30 last:border-0 border-l-2 border-transparent hover:border-[var(--color-primary)]">
                                <div className="flex items-center gap-5">
                                    <div className="size-11 rounded-xl bg-[var(--bg-surface-muted)]/20 border border-[var(--border-muted)] flex items-center justify-center text-[var(--color-primary)] font-display font-black shadow-inner shadow-black/20 group-hover/row:scale-105 transition-transform">{corp.initial}</div>
                                    <div className="space-y-0.5">
                                        <p className="font-display font-bold text-base text-white/90 leading-tight group-hover/row:text-[var(--color-primary)] transition-colors">{corp.name}</p>
                                        <p className="text-[8px] text-[var(--text-muted)] font-black uppercase tracking-[0.15em] opacity-40 italic">{corp.location}</p>
                                    </div>
                                </div>
                                <p className="font-display font-black text-xl text-[var(--color-primary)] tracking-tighter opacity-80">{formatCurrency(corp.revenue)}</p>
                            </div>
                        )) : (
                ...
                <motion.div
                    ...
                    className="lg:col-span-5 rounded-[var(--r-lg)] border border-[var(--border-muted)] bg-[var(--bg-card)]/40 p-1 backdrop-blur-sm overflow-hidden group shadow-sm flex flex-col min-w-0"
                >
                    <div className="p-8">
                        ...
                        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="p-4 rounded-2xl bg-[var(--bg-card)]/[0.02] border border-white/[0.05] flex flex-col justify-center items-center text-center">
                                ...
                                <p className={`text-2xl font-display font-black ${stats.bizGrowth >= 0 ? 'text-emerald-500' : 'text-red-400'}`}>{stats.bizGrowth >= 0 ? '+' : ''}{stats.bizGrowth}%</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
            ...
            <motion.div
                ...
                viewport={{ once: true }}
                className="rounded-[var(--r-lg)] border border-[var(--border-muted)] bg-[var(--bg-card)]/40 overflow-hidden shadow-sm flex flex-col mb-12 backdrop-blur-sm"
            >
                <div className="p-8 border-b border-[var(--border-muted)] flex justify-between items-center bg-black/10">
                    <div className="space-y-1">
                        <h3 className="text-xl font-display font-bold text-[var(--text-main)]">Administrative Activity Feed</h3>
                                ...
                                <div key={activity.id} className="flex gap-5 p-4 rounded-2xl bg-[var(--bg-card)]/[0.02] border border-white/[0.05] group hover:border-[var(--color-primary)]/30 transition-all">
                                    ...
                                    <div className="flex-1 flex flex-col justify-center min-w-0">
                                        <div className="flex items-center justify-between w-full gap-4">
                                            <p className="text-sm font-display font-bold text-white/80 truncate">{activity.user.fullName}</p>
