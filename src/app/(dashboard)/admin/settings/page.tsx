        ...
        <div className="flex h-full w-full flex-col lg:flex-row bg-[var(--bg-app)] text-[var(--text-main)] transition-colors duration-500 overflow-hidden">
            ...
            <aside className="w-full lg:w-85 glass-card p-10 space-y-12 flex-shrink-0 hidden lg:flex flex-col border-r border-[var(--border-muted)] bg-[var(--bg-surface-muted)]/20 shadow-none">
                <div className="space-y-8">
                    <div className="px-2">
                        <h3 className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em] mb-8 opacity-40">System Architect</h3>
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <p className="text-[9px] font-bold text-[var(--text-muted)] uppercase tracking-widest mt-2 opacity-60">{session.user.role === 'ADMIN' ? 'Compliance Architect' : 'Sanctuary Guardian'}</p>
                            </div>
                        </div>
                    </div>

                    <nav className="flex flex-col gap-2">
                        <Link href="/admin/dashboard" className="flex items-center gap-4 px-6 py-4 text-sm font-display font-bold rounded-[1.5rem] text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface-muted)]/30 transition-all tracking-tight group">
                            <span className="material-symbols-outlined text-xl opacity-40 group-hover:scale-110 transition-transform">language</span>
                            <span>Global Pulse</span>
                        </Link>
                        <Link href="/admin/settings" className="flex items-center gap-4 px-6 py-4 text-sm font-display font-bold rounded-[1.5rem] bg-[var(--color-primary)]/10 text-[var(--color-primary)] shadow-sm border border-[var(--color-primary)]/20 tracking-tight group">
                            <span className="material-symbols-outlined text-xl font-bold icon-filled">settings_input_component</span>
                            <span>Regional Presets</span>
                        </Link>
                        <Link href="/admin/audit" className="flex items-center gap-4 px-6 py-4 text-sm font-display font-bold rounded-[1.5rem] text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface-muted)]/30 transition-all tracking-tight group">
                            <span className="material-symbols-outlined text-xl opacity-40 group-hover:scale-110 transition-transform">lock_person</span>
                            <span>Compliance Vault</span>
                        </Link>
                        <Link href="/admin/analytics" className="flex items-center gap-4 px-6 py-4 text-sm font-display font-bold rounded-[1.5rem] text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface-muted)]/30 transition-all tracking-tight group">
                            ...
                            <span>Tax Harmonics</span>
                        </Link>
                        <Link href="/admin/legal" className="flex items-center gap-4 px-6 py-4 text-sm font-display font-bold rounded-[1.5rem] text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-surface-muted)]/30 transition-all tracking-tight group">
                            <span className="material-symbols-outlined text-xl opacity-40 group-hover:scale-110 transition-transform">description</span>
                            <span>Legal Weave</span>
                        </Link>
                    </nav>
                </div>
                ...
                <div className="mt-auto space-y-6">
                    <div className="bg-[var(--bg-app)]/50 rounded-[2.5rem] p-8 border border-[var(--border-muted)] shadow-inner relative overflow-hidden group/health">
                        ...
                        <p className="text-[10px] font-bold text-[var(--color-primary)] mb-4 uppercase tracking-[0.2em] opacity-60">System Resonance</p>
                        ...
                        <p className="text-[9px] font-bold text-[var(--text-muted)] uppercase tracking-widest opacity-40">All nodes calibrated</p>
                    </div>
                </div>
            </aside>
            ...
            <section className="flex-1 p-6 lg:p-12 max-w-7xl mx-auto w-full overflow-y-auto no-scrollbar scroll-smooth">
                <div className="mb-12 space-y-3">
                    <h1 className="text-5xl font-display font-bold tracking-tight text-[var(--text-main)]">Regional <span className="text-[var(--color-primary)] underline decoration-2 decoration-[var(--color-primary)]/20 underline-offset-8">Tuning</span></h1>
                    <p className="text-[var(--text-muted)] mt-5 max-w-3xl text-xl font-bold leading-relaxed">Manage international regulatory harmonics and automated fiscal logic for global expansion from a singular sanctuary.</p>
                </div>
                        ...
                        <div className="glass-card border border-[var(--border-muted)] overflow-hidden rounded-[2.5rem] shadow-2xl shadow-[var(--color-primary)]/5">
                            <div className="p-8 border-b border-[var(--border-muted)] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-[var(--bg-surface-muted)]/30 backdrop-blur-xl">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] opacity-60">Architectural Focal Point</p>
                                </div>
                ...
                <div className="mt-16 mb-12 glass-card border border-[var(--border-muted)] overflow-hidden rounded-[2.5rem] bg-[var(--bg-card)] shadow-2xl shadow-black/5">
                    <div className="p-10 border-b border-[var(--border-muted)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 bg-[var(--bg-surface-muted)]/20">
                        ...
                        <Link href="/admin/audit" className="group flex items-center gap-3 px-8 py-3 bg-[var(--bg-card)] hover:bg-[var(--text-main)] hover:text-[var(--bg-app)] border border-[var(--border-muted)] transition-all rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] shadow-sm active:scale-[0.98]">
                            View Archival Vault
