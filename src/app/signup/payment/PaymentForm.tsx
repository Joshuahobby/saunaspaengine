...
type Stage = "idle" | "loading" | "pending" | "error";

function fmt(n: number) {
    return n.toLocaleString("en-RW");
}

export function PaymentForm({ email, amount, plan, cycle }: PaymentFormProps) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            const data = await res.json() as { depositId?: string; error?: string };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="bg-[var(--bg-surface-muted)] rounded-xl p-4 border border-[var(--border-muted)] flex items-start gap-3">
                    ...
                    <p className="text-xs text-[var(--text-muted)] font-medium leading-relaxed">
                        You will receive a USSD push on your phone. Approve the prompt to complete payment. Your account activates instantly — no waiting for manual review.
                    </p>
                </div>
                ...
                >
                    {stage === "loading" ? (
                        ...
                        <>
                            <span className="material-symbols-outlined font-black">send_money</span>
                            Pay RWF {fmt(amount)} via MoMo
                        </>
                    )}
                </button>
            </form>
