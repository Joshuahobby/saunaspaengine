    ...
    return (
        <main className="flex-1">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="glass-card p-8 rounded-2xl border border-[var(--border-muted)]">
                            ...
                            <form className="space-y-4" onSubmit={e => { e.preventDefault(); window.location.href = "/demo/confirmation"; }}>
                                ...
                                <div className="flex flex-col gap-1"><label className="text-sm font-bold text-[var(--text-muted)]">Work Email *</label><input className="w-full rounded-lg border-[var(--border-muted)] bg-[var(--bg-app)] focus:ring-[var(--color-primary)] px-4 py-3" type="email" placeholder="jane@myspabusiness.com" required /></div>
                                <div className="flex flex-col gap-1"><label className="text-sm font-bold text-[var(--text-muted)]">Spa Business Name</label><input className="w-full rounded-lg border-[var(--border-muted)] bg-[var(--bg-app)] focus:ring-[var(--color-primary)] px-4 py-3" placeholder="Serenity Wellness Center" /></div>
                                <div className="flex flex-col gap-1"><label htmlFor="locations" className="text-sm font-bold text-[var(--text-muted)]">Number of Locations</label><select id="locations" title="Select Number of Locations" aria-label="Number of business locations" className="w-full rounded-lg border-[var(--border-muted)] bg-[var(--bg-app)] focus:ring-[var(--color-primary)] px-4 py-3"><option>1 Location</option><option>2-5 Locations</option><option>6-20 Locations</option><option>20+ Locations</option></select></div>
            ...
            <section className="py-16 max-w-6xl mx-auto px-6">
                <h2 className="text-2xl font-bold mb-8 text-center">Trusted by 500+ Spa Businesses Worldwide</h2>
